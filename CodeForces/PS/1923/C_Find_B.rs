...
fn solve<R: BufRead, W: Write>(mut input: FastInput<R>, mut w: W) {
    ...
    for _ in 0..t {
        ...
        let mut onecount: Vec<i64> = vec![0i64; n + 1];
        for i in 1..=n {
            onecount[i] = onecount[i - 1] + (a[i - 1] == 1) as i64;
        }
        let mut ps: Vec<i64> = vec![0i64; n + 1];
        for i in 1..=n {
            ps[i] = ps[i - 1] + a[i - 1] as i64;
        }
        for query in 0..q {
            let l: usize = input.next();
            let r: usize = input.next();
            let sum: i64 = ps[r] - ps[l - 1];
            let oc = onecount[r] - onecount[l - 1];
            let nn = (r - l + 1) as i64;
            if l == r || (nn + oc > sum) {
                writeln!(w, "NO");
            } else {
