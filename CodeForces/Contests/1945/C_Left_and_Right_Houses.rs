...
fn solve<R: BufRead, W: Write>(mut input: FastInput<R>, mut w: W) {
    ...
    'test: for _ in 0..t {
        ...
        let mut a: Vec<u8> = input.next();
        for x in a.iter_mut() {
            *x -= b'0';
        }
        let mut ps: Vec<usize> = vec![0; n + 1];
        for i in 1..=n {
            ps[i] = ps[i - 1] + a[i - 1] as usize;
        }
        let mut ans = u64::MAX as usize;
        let mp = n as f64 / 2.0;
        if n % 2 == 1 {
            for step in (1..=n).step_by(2) {
                let pos = (mp - (step as f64) / 2.0).round() as usize;
                let l = ps[pos];
                let r = ps[n] - ps[pos];
                if l <= pos / 2 && r >= (n - pos + 1) / 2 {
                    ans = pos;
                    break;
                }
                let pos = (mp + (step as f64) / 2.0).round() as usize;
