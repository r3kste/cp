...
fn no_of_trailing_zeros(n: i64) -> i64 {
    let mut count = 0;
    let mut i = 10;
    while n % i == 0 {
        count += 1;
...
fn solve<R: BufRead, W: Write>(input: &mut FastInput<R>, w: &mut W) {
    ...
    let mut a: Vec<i64> = (0..n).map(|_| input.next()).collect();
    a.sort_by_cached_key(|&x| no_of_trailing_zeros(x));

    let mut total = a.iter().map(|&x| no_of_digits(x)).sum::<i64>();
    let mut ptr = n - 1;

    while ptr >= 0 && no_of_trailing_zeros(a[ptr as usize]) > 0 {
        total -= no_of_trailing_zeros(a[ptr as usize]);
        ptr -= 2;
    }
