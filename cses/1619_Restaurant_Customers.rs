...
fn solve<R: BufRead, W: Write>(input: &mut FastInput<R>, w: &mut W) {
    ...
    let customers: Vec<(i64, i64)> = (0..n).map(|_| (input.next(), input.next())).collect();
    let mut people: HashMap<i64, i64> = HashMap::new();

    for customer in customers.iter() {
        let (a, b) = customer;
        *people.entry(*a).or_insert(0) += 1;
        *people.entry(*b).or_insert(0) -= 1;
    }

    let mut times = Vec::new();
    for (arrival, depart) in customers {
        times.push(arrival);
        times.push(depart);
    }
    ...
    for time in times {
        current_people += people.get(&time).unwrap();
        max_people = max_people.max(current_people);
    }

    writeln!(w, "{}", max_people);
}
