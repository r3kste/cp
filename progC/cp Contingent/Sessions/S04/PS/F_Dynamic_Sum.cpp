...
struct SegTree {
    ...
    void point_update(int target, T value, int vertex, range borders) {
        if (borders.L == borders.R) {
            ...
            if (target <= mid) {
                point_update_operation(target, value, vertex << 1, {borders.L, mid});
            } else {
                point_update_operation(target, value, vertex << 1 | 1, {mid + 1, borders.R});
            }
    ...
    void Update(int target, T value) {
        point_update_operation(target, value, 1, {1, n});
    }
